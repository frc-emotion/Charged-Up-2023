...
public class LevelChargeStation extends CommandBase {
    ...
    private static final TrapezoidProfile.Constraints travelConstraints = new TrapezoidProfile.Constraints(
            DriveConstants.MAX_LEVEL_VELOCITY, DriveConstants.MAX_LEVEL_ACCELERATION);
    private final ProfiledPIDController angleController = new ProfiledPIDController(DriveConstants.KPLevel,
            DriveConstants.KILevel, DriveConstants.KDLevel, travelConstraints); // General approximation should have Kp
                                                                                // be something like Max Velocity / max
                                                                                // Angle
    ...
    public void initialize() {
        angleController.setGoal(DriveConstants.TARGET_ANGLE); // In this version which only uses one PID loop goal is
                                                              // set immediately since it does not change
    }
    ...
    public boolean isFinished() {
        double error1 = (swerve.getPitch()); // Returns measure of Roll and Pitch and takes cos and sin to determine error
        if (Math.abs(error1) < DriveConstants.THRESHOLD) {
            return true;
        } else {
            // return Math.abs(DriveConstants.TARGET_ANGLE - -swerve.getPitch() *
            // Math.cos(Units.degreesToRadians(swerve.getHeading()))) - (swerve.getRoll() *
            // Math.sin(Units.degreesToRadians(swerve.getHeading()))) <
            // DriveConstants.THRESHOLD; //Maybe have it end the command when the specified
            // threshold is met and the robot is 'flat'?
            return false;
        }
    }
}